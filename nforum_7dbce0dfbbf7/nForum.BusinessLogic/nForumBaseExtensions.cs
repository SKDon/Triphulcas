        ...
        public static string NewForumPost(string topicId)
        {
            ...
            if (MembershipHelper.IsAuthenticated())
            {
                ...
                if (!String.IsNullOrEmpty(Helpers.SafePlainText(topicId).Trim()))
                {
                    ...
                    // Create the new post
                    var u = new User(0);
                    var d = Document.MakeNew(title, DocumentType.GetByAlias("ForumPost"), u, parentTopicId);
            ...
            if (MembershipHelper.IsAuthenticated())
            {
                ...
                if (!String.IsNullOrEmpty(Helpers.SafePlainText(catId).Trim()))
                {
                    if (!string.IsNullOrWhiteSpace(tbPost) && !string.IsNullOrWhiteSpace(tbTitle))
                    {
                        ...
                        // Create the topic
                        var t = Document.MakeNew(topicTitle, DocumentType.GetByAlias("ForumTopic"), u, catId.ToInt32());
                        ...
                        // Create the post
                        var p = Document.MakeNew(topicTitle, DocumentType.GetByAlias("ForumPost"), u, t.Id);
