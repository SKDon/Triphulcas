...
(function () {
    var gs = window.gs = { player: null, DEBUG: false, fakeConsole: null, fakeConsoleBox: null, rootUrl: location.protocol + "//" + location.host, pageUrl: location.protocol + "//" + location.host + location.pathname, scriptUrl: location.href };
    gs.debug = function () {
        if (gs.DEBUG || gs.is_dev) {
            if (!(typeof window.console == 'undefined')) {
                ...
                if (!gs.fakeConsole) {
                    $(document.body).append("<div id='fakeConsole' style='display:none'><div class='close' onclick='$(this).click(function(){$(this).parent().hide()})'>X</div><div class='msg'></div></div>");
                    gs.fakeConsole = document.getElementById("fakeConsole");
                    gs.fakeConsoleBox = gs.fakeConsole.childNodes[1];
                    $.hotkeys.add('ctrl+shift+z', function (event) {
                        $(gs.fakeConsole).toggle();
                    });
                }
                for (var i = 0; i < arguments.length; i++) {
                    $(gs.fakeConsoleBox).append(arguments[i] + "<br><br>");
                }
            }
        }
    };
    window.debug = gs.debug;
    gs.util = {
        isUndefined: function (a) {
            ...
            return gs.util.isObject(a) && typeof a.constructor != 'function';
        }, isObject: function (a) {
            ...
            return gs.util.isObject(a) && a.constructor == Array;
        }, isEmpty: function (o) {
    ...
    gs.collapseModule = function (href) {
        href.blur();
        $(href).toggleClass('closed');
        $(href).parent().next('.collapseable').slideToggle('slow').toggleClass('closed');
        return false;
    }
    gs.createImageHover = function (find) {
        $(find).css('opacity', 0.01);
        $(find).hover(function () {
            $(this).animate({ opacity: 0.5, borderWidth: "5px" }, 300);
        }, function () {
            $(this).animate({ opacity: 0.01, borderWidth: "0px" }, 300);
        });
    }
