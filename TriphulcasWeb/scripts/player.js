...
function jsPlayer(hostname, callbacks) {
    var self = this;
    var swf;
    var eiAttempts = 0;
    this.songID = 0;
    ...
    this.init = function () {
        ...
        var attributes = { id: 'jsPlayerEmbed', name: 'jsPlayerEmbed' };
        var swfWrapper = 'swfWrapper';
        if (!document.getElementById('swfWrapper')) {
            $(document.body).append('<div id="swfWrapper"></div>');
        }
        swfobject.embedSWF("http://listen.grooveshark.com/JSPlayer.swf", "swfWrapper", "1", "1", "9.0.0", null, vars, params, attributes);
    ...
    this.playerStatus = function (msg) {
        debug('player status CALLBACK', msg);
        switch (msg) {
            case 'playing':
                debug('i play');
                $('#sr-' + this.songID).addClass('pause').removeClass('loading').removeClass('play');
                break;
            case 'loading':
            case 'buffering':
                debug('i ' + msg);
                $('#sr-' + this.songID).addClass('loading').removeClass('play').removeClass('pause');
                break;
            case 'completed':
            case 'paused':
            case 'failed':
            default:
                debug('i ' + msg, this.songID);
                $('#sr-' + this.songID).addClass('play').removeClass('loading').removeClass('pause');
                break;
        }
    }
